        ...
        <div class="col-lg-6">
            <div class="card card-default shadow-sm">
                ...
                <?php foreach ($pemeliharaan as $pm) { ?>
                    <div class="card-body">
                        <form action="<?= base_url('pemeliharaan/proses_edit/') . $pm->id_pemeliharaan ?>" method="POST">
                            <div class="form-row">
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <input type="date" class="form-control" name="tanggal_mulai" value="<?= $pm->tgl_mulai ?>">
                                </div>

                                <div class="col-md-12 mb-3">
                                    <label>Tanggal Selesai</label>

                                    <input type="date" class="form-control" name="tanggal_selesai" value="<?= $pm->tgl_selesai ?>">
                                </div>

                                <div class="col-md-12 mb-3">
                                    <label>Deskripsi</label>
                                    <input type="text" class="form-control" placeholder="Kegiatan" name="deskripsi" value="<?= $pm->deskripsi ?>">
                                </div>

                                <div class="col-md-12 mb-3">
                                    <label for="validationServerUsername">Teknisi</label>
                                    <input type="hidden" name="id_user" value="<?= $_SESSION['id_user'] ?>">
                                    <input type="text" readonly class="form-control" value="<?= $pm->nama ?>">
                                </div>
                            </div>
                    </div>
            </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select class="form-control" id="ruangan" name="ruangan">
                                ...
                                <?php foreach ($ruang as $row) { ?>
                                    <option <?php if ($row->id_ruangan == $pm->id_ruangan) {
                                                echo "selected";
                                            } ?> value="<?= $row->id_ruangan; ?>"><?= $row->nama_ruangan; ?></option>
                                <?php } ?>
                            </select>
                        </div>
